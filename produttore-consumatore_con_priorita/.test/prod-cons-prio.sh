...
perl -n -e 'BEGIN{$counter=0; $coda1=0; $coda2=0; %prod1=(); %prod2=(); %cons=(); @produced=(); @consumed=(); } if(/Produttore\stipo\s(\d)\scon\spid\s\d+\saccede\sal\smonitor/) { if($1==1) {$coda1++;} else {$coda2++;} } if(/Produttore\stipo\s(\d)\scon\spid\s\d+\sha\sprodotto\s(\d+)/) { push @produced,$2; if($1==1) {$coda1--;} else { if($coda1>0) { exit(1); } $coda2--;} } if(/Consumatore\scon\spid\s\d+\sha\sconsumato\s(\d+)/) { push @consumed,$1; } if($counter>3 || $counter<0) { exit(1); } END { if($#produced!=$#consumed) { exit(1); }  for $i(0..$#produced) { if($produced[$i]!=$consumed[$i]){ exit(1); } } }' $OUTPUT
